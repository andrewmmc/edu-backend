...
class Payments extends CI_Controller
{
	...
	function Pay_chained()
	{
		...
		$transaction_id = $this->Transaction_records_model->insert_paypal_records($courses_id, $providers_id, $members_id, $amounts, $ref_id);

		// Prepare request arrays
		$PayRequestFields = array(
			'ActionType' => 'PAY',                                // Required.  Whether the request pays the receiver or whether the request is set up to create a payment request, but not fulfill the payment until the ExecutePayment is called.  Values are:  PAY, CREATE, PAY_PRIMARY
			'CancelURL' => site_url('paypal/Payments/pay_cancel/' . $transaction_id),                                    // Required.  The URL to which the sender's browser is redirected if the sender cancels the approval for the payment after logging in to paypal.com.  1024 char max.
			...
			'ReturnURL' => site_url('paypal/Payments/pay_return/' . $transaction_id),                                    // Required.  The URL to which the sener's browser is redirected after approvaing a payment on paypal.com.  1024 char max.
		...
		$Receivers = array();
		$Receiver = array(
			'Amount' => $amounts,                                            // Required.  Amount to be paid to the receiver.
			//'Amount' => '100.00',                                            // Required.  Amount to be paid to the receiver.
			'Email' => '12104585d@connect.polyu.hk',                                                // Receiver's email address. 127 char max.
			'InvoiceID' => $ref_id,                                            // The invoice number for the payment.  127 char max.
