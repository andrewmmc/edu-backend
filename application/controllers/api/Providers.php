...
class Providers extends REST_Controller
{
	...
	{
		$data = $this->post();
		$result = $this->Courses_lessons_model->update_lessons($data['lessons_id'], $data['start_time'], $data['end_time'], $data['status']);
		...
		if ($courses_id === NULL) {
			...
			for ($i = 0; $i < sizeof($lessons); $i++) {
				...
				for ($j = 0; $j < sizeof($registration); $j++) {
					...
					$attended = $this->Courses_attendance_model->get_members_lessons_attendance($registration[$j]->members_id, $lessons[$i]->lessons_id);
					if (!empty($attended)) {
